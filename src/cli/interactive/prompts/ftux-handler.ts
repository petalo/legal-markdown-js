...
async function handleSetupConfiguration(): Promise<string> {
  ...
  // Ask for styles directory
  const stylesDir = await input({
    message: 'Where are your CSS stylesheets stored? (styles directory)',
    default: path.join(path.resolve(inputDir), 'styles'),
    validate: value => {
      const resolvedPath = path.resolve(value);
      const parentDir = path.dirname(resolvedPath);
      if (!fs.existsSync(parentDir)) {
  ...
  // Ask for archive directory
  const archiveDir = await input({
    message: 'Where should processed input files be archived? (archive directory)',
    default: path.join(path.resolve(outputDir), 'archive'),
    validate: value => {
      const resolvedPath = path.resolve(value);
      const parentDir = path.dirname(resolvedPath);
      if (!fs.existsSync(parentDir)) {
...
`;
  ...
  if (fs.existsSync(envPath)) {
    ...
    if (!overwrite) {
      console.log(chalk.yellow('⚠️  Setup cancelled. Existing configuration unchanged.'));
      console.log(chalk.gray('Returning to main menu...\n'));
      // Return to FTUX main menu instead of exiting
      return await handleFirstTimeUserExperience();
    } else {
  ...
  const resolvedStylesDir = path.resolve(stylesDir);
  ...
  if (!fs.existsSync(resolvedStylesDir)) {
    fs.mkdirSync(resolvedStylesDir, { recursive: true });
    console.log(chalk.green(`✅ Created styles directory: ${resolvedStylesDir}`));
  }

  if (!fs.existsSync(resolvedArchiveDir)) {
    fs.mkdirSync(resolvedArchiveDir, { recursive: true });
    console.log(chalk.green(`✅ Created archive directory: ${resolvedArchiveDir}`));
  }
